//Could not get pros to work so it has been coded out, but not 
// deleted for further development later.
...
const Create = () => {
  ...
  const handleFormSubmit = async event => {
    ...
    try {
      ...
      await addDecision({
        variables: { name, decisionText }
      })
      // removed pros, cons from variables
      // try {
            //   await addPro({
            //     variables: { ...pros }
            //   }) 
